...
def cmd_start(msg: types.Message):
    ...
    if not existing:
        ...
        # ── NEW USER NOTIFICATION TO OWNER ──────────────
...
def cmd_ban(msg: types.Message):
    ...
    try:
        ...
        if not user:
            ...
            return
        if user["is_banned"]:
            bot.send_message(msg.chat.id, f"⚠️ User <code>{uid}</code> is already banned.", parse_mode="HTML")
            return
        db_update_user(uid, is_banned=1)
        bot.send_message(
            msg.chat.id,
            f"⛔ User <code>{uid}</code> has been <b>banned</b>.",
            parse_mode="HTML",
        )
        safe_send(uid, "⛔ You have been banned by admin.")
        logger.info("User %s banned by admin.", uid)
    except (ValueError, IndexError):
...
def cmd_unban(msg: types.Message):
    ...
    try:
        ...
        if not user["is_banned"]:
            bot.send_message(msg.chat.id, f"⚠️ User <code>{uid}</code> is not banned.", parse_mode="HTML")
            return
        db_update_user(uid, is_banned=0)
        bot.send_message(
            msg.chat.id,
            f"✅ User <code>{uid}</code> has been <b>unbanned</b>.",
            parse_mode="HTML",
        )
        safe_send(uid, "✅ You have been unbanned by admin.")
        logger.info("User %s unbanned by admin.", uid)
    except (ValueError, IndexError):
...
def cmd_add(msg: types.Message):
    ...
    try:
        ...
        bot.send_message(
            msg.chat.id,
            f"✅ Added <b>{amount} TK</b> to user <code>{uid}</code>.\n"
            f"New balance: <b>{new_bal} TK</b>",
            parse_mode="HTML",
        )
        safe_send(uid, f"💰 Admin added <b>{amount} TK</b> to your balance.")
        logger.info("Admin added %s TK to user %s. New balance: %s", amount, uid, new_bal)
    except (ValueError, IndexError):
        bot.send_message(msg.chat.id, "Usage: /add <user_id> <amount>")


@bot.message_handler(commands=["remove"])
def cmd_remove(msg: types.Message):
    """Admin: /remove <user_id> <amount>"""
    if msg.from_user.id != OWNER_ID:
        return
    try:
        _, uid_str, amount_str = msg.text.split()
        ...
        bot.send_message(
            msg.chat.id,
            f"✅ Removed <b>{amount} TK</b> from user <code>{uid}</code>.\n"
            f"New balance: <b>{new_bal} TK</b>",
            parse_mode="HTML",
        )
        safe_send(uid, f"💸 Admin removed <b>{amount} TK</b> from your balance.")
        logger.info("Admin removed %s TK from user %s. New balance: %s", amount, uid, new_bal)
    except (ValueError, IndexError):
        bot.send_message(msg.chat.id, "Usage: /remove <user_id> <amount>")
...
def cmd_check(msg: types.Message):
    ...
    try:
        ...
        banned_display          = "Yes" if user["is_banned"] else "No"
        pending_task_display    = "Yes" if pending_activation > 0 else "No"
        ...
        text = (
            ...
            f"📛 <b>Name:</b>          {user['first_name']}\n"
            f"💰 <b>Balance:</b>       {user['balance']} TK\n"
            f"✅ <b>Tasks Done:</b>    {user['referrals']}\n"
            f"💸 <b>Withdrawals:</b>   {approved_withdrawals}\n"
            f"👥 <b>Referrals:</b>     {user['referrals']}\n"
            f"⏳ <b>Pending Task:</b>  {pending_task_display}\n"
            f"⛔ <b>Banned:</b>        {banned_display}\n"
        )
        bot.send_message(msg.chat.id, text)
        logger.info("Admin checked user %s.", uid)
    except (ValueError, IndexError):
        bot.send_message(msg.chat.id, "Usage: /check <user_id>")
